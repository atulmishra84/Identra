package com.identra.adapter.midpoint;
...
/** MidPoint adapter (v3) — dry-run surface for catalog completeness. */
public class MidPointIdentityAdapter implements IdentityAdapter, HealthCapable {

    public static final String ADAPTER_ID = "midpoint";
    private static final Logger log = LoggerFactory.getLogger(MidPointIdentityAdapter.class);
    private final UUID defaultTenantId;

    public MidPointIdentityAdapter() {
        this(UUID.fromString("11111111-1111-1111-1111-111111111111"));
    }

    public MidPointIdentityAdapter(UUID defaultTenantId) {
    ...
    public Identity create(Identity identity) {
        log.info("MidPoint dry-run create {}", identity.userName());
        return canonical(ADAPTER_ID + "-dry-" + UUID.randomUUID(), identity.userName());
    }

    @Override
    public Identity update(Identity identity) {
        log.info("MidPoint dry-run update {}", identity.userName());
        return identity;
    }

    @Override
    public void disable(String externalId) {
        log.info("MidPoint dry-run disable {}", externalId);
    }

    @Override
    public void delete(String externalId) {
        log.info("MidPoint dry-run delete {}", externalId);
    }
