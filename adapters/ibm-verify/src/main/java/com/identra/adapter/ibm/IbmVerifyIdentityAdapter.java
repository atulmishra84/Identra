package com.identra.adapter.ibm;
...
/** IBM Security Verify adapter (v3) — dry-run surface for catalog completeness. */
public class IbmVerifyIdentityAdapter implements IdentityAdapter, HealthCapable {

    public static final String ADAPTER_ID = "ibm-verify";
    private static final Logger log = LoggerFactory.getLogger(IbmVerifyIdentityAdapter.class);
    private final UUID defaultTenantId;

    public IbmVerifyIdentityAdapter() {
        this(UUID.fromString("11111111-1111-1111-1111-111111111111"));
    }

    public IbmVerifyIdentityAdapter(UUID defaultTenantId) {
    ...
    public Identity create(Identity identity) {
        log.info("IBM Security Verify dry-run create {}", identity.userName());
        return canonical(ADAPTER_ID + "-dry-" + UUID.randomUUID(), identity.userName());
    }

    @Override
    public Identity update(Identity identity) {
        log.info("IBM Security Verify dry-run update {}", identity.userName());
        return identity;
    }

    @Override
    public void disable(String externalId) {
        log.info("IBM Security Verify dry-run disable {}", externalId);
    }

    @Override
    public void delete(String externalId) {
        log.info("IBM Security Verify dry-run delete {}", externalId);
    }
    ...
    private Identity canonical(String externalId, String userName) {
        ...
        return new Identity(
                ...
                Map.of("vendor", "IBM Security Verify"),
                ADAPTER_ID,
                0L,
                "W/\"" + externalId + "\"",
                now,
