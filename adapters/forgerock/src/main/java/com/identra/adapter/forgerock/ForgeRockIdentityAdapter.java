package com.identra.adapter.forgerock;
...
/** ForgeRock adapter (v3) — dry-run surface for catalog completeness. */
public class ForgeRockIdentityAdapter implements IdentityAdapter, HealthCapable {

    public static final String ADAPTER_ID = "forgerock";
    private static final Logger log = LoggerFactory.getLogger(ForgeRockIdentityAdapter.class);
    private final UUID defaultTenantId;

    public ForgeRockIdentityAdapter() {
        this(UUID.fromString("11111111-1111-1111-1111-111111111111"));
    }

    public ForgeRockIdentityAdapter(UUID defaultTenantId) {
    ...
    public Identity create(Identity identity) {
        log.info("ForgeRock dry-run create {}", identity.userName());
        return canonical(ADAPTER_ID + "-dry-" + UUID.randomUUID(), identity.userName());
    }

    @Override
    public Identity update(Identity identity) {
        log.info("ForgeRock dry-run update {}", identity.userName());
        return identity;
    }

    @Override
    public void disable(String externalId) {
        log.info("ForgeRock dry-run disable {}", externalId);
    }

    @Override
    public void delete(String externalId) {
        log.info("ForgeRock dry-run delete {}", externalId);
    }
