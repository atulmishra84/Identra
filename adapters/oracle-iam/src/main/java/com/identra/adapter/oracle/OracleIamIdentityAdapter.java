package com.identra.adapter.oracle;
...
/** Oracle IAM adapter (v3) — dry-run surface for catalog completeness. */
public class OracleIamIdentityAdapter implements IdentityAdapter, HealthCapable {

    public static final String ADAPTER_ID = "oracle-iam";
    private static final Logger log = LoggerFactory.getLogger(OracleIamIdentityAdapter.class);
    private final UUID defaultTenantId;

    public OracleIamIdentityAdapter() {
        this(UUID.fromString("11111111-1111-1111-1111-111111111111"));
    }

    public OracleIamIdentityAdapter(UUID defaultTenantId) {
    ...
    public Identity create(Identity identity) {
        log.info("Oracle IAM dry-run create {}", identity.userName());
        return canonical(ADAPTER_ID + "-dry-" + UUID.randomUUID(), identity.userName());
    }

    @Override
    public Identity update(Identity identity) {
        log.info("Oracle IAM dry-run update {}", identity.userName());
        return identity;
    }

    @Override
    public void disable(String externalId) {
        log.info("Oracle IAM dry-run disable {}", externalId);
    }

    @Override
    public void delete(String externalId) {
        log.info("Oracle IAM dry-run delete {}", externalId);
    }
