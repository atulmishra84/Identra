...
public class AdapterDispatchService {
    ...
    public AdapterDispatchService(
        ...
        adapters.put(
                ...
                new OktaIdentityAdapter(new OktaClientConfig(oktaOrgUrl, oktaToken, oktaDryRun),
                        java.util.UUID.fromString("11111111-1111-1111-1111-111111111111"))
        );
        adapters.put(
                SailPointIscIdentityAdapter.ADAPTER_ID,
                new SailPointIscIdentityAdapter(
                        new SailPointIscClientConfig(iscBaseUrl, "client", iscToken, iscDryRun),
                        java.util.UUID.fromString("11111111-1111-1111-1111-111111111111"))
        );
    }
    ...
    public AdapterDispatchResult dispatch(AdapterDispatchRequest request) {
        ...
        ProvisioningRequest.Operation op = request.operation();
        return switch (op) {
            case CREATE -> {
                Identity created = adapter.create(request.identity());
                yield new AdapterDispatchResult("SUCCEEDED", "Created via " + request.adapterId(), created);
            }
            case UPDATE, ENABLE -> {
                Identity updated = adapter.update(request.identity());
                yield new AdapterDispatchResult("SUCCEEDED", "Updated via " + request.adapterId(), updated);
            }
            case DISABLE -> {
            ...
            case RESET_PASSWORD, ASSIGN_ENTITLEMENT, REVOKE_ENTITLEMENT ->
                    new AdapterDispatchResult(
                            "SUCCEEDED",
                            "Operation " + op + " accepted (entitlement/password handlers follow-up)",
                            null
                    );
        };
    }
